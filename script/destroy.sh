...
# 다음 apply 가 깨끗한 1차 apply 부터 시작하도록 비워둠.
sed -i -E \
  -e 's|^alb_listener_arn = ".*"|alb_listener_arn = ""|' \
  -e 's|^cloudfront_domain = ".*"|cloudfront_domain = ""|' \
  "${INFRA_TFVARS}"
echo ">>> tfvars 초기화: alb_listener_arn / cloudfront_domain 빈 값으로"
